...
<template>
  ...
  >
    <v-layout>
      <v-app-bar color="grey-lighten-3" elevation="1">
        <v-spacer></v-spacer>

        <v-btn v-tooltip="'open source in tab'" class="ma-2 bg-grey-lighten-2" icon size="x-small"
      ...
      <v-main>
        <v-container>
          <v-row>
            <blockquote class="mx-2">
              {{ quote }}
            </blockquote>
          </v-row>
          <v-row class="justify-end">
            <p class="text-grey-darken-1 mx-2"><span class="text-grey-lighten-1">source: </span>{{ author }}</p>
          </v-row>
        </v-container>
      </v-main>
    </v-layout>
